...
pub const ID_TERMINAL_IDENTIFIER: u32 = 0x0009;
...
pub const ID_TERMINAL_COMPARISON_OP: u32 = 0x000B;
...
pub const ID_VARIABLE_EXP_ATOM: u32 = 0x000C;
/// The unique identifier for variable `exp_factor`
pub const ID_VARIABLE_EXP_FACTOR: u32 = 0x000D;
/// The unique identifier for variable `exp_term`
pub const ID_VARIABLE_EXP_TERM: u32 = 0x000E;
/// The unique identifier for variable `exp`
pub const ID_VARIABLE_EXP: u32 = 0x000F;
...
pub trait Visitor {
    ...
    fn on_terminal_type(&self, node: &AstNode) {}
    fn on_terminal_comparison_op(&self, node: &AstNode) {}
...
pub fn visit_ast_node(node: AstNode, visitor: &dyn Visitor) {
    ...
    match node.get_symbol().id {
        ...
        0x0009 => visitor.on_terminal_identifier(&node),
        0x000A => visitor.on_terminal_type(&node),
        0x000B => visitor.on_terminal_comparison_op(&node),
        0x000C => visitor.on_variable_exp_atom(&node),
        0x000D => visitor.on_variable_exp_factor(&node),
        0x000E => visitor.on_variable_exp_term(&node),
        0x000F => visitor.on_variable_exp(&node),
        _ => ()
    };
}
